##############################################################################
# JSONRPC version 1.1 / patched by foo1inge
...
package Datacoin::JSON::RPC::Client;

$Datacoin::JSON::RPC::Client::VERSION = '0.94';
...
sub AUTOLOAD {
    my $self   = shift;
    my $method = $Datacoin::JSON::RPC::Client::AUTOLOAD;
...
sub new {
    ...
    my $ua  = LWP::UserAgent->new(
        agent   => 'Datacoin::JSON::RPC::Client/' . $Datacoin::JSON::RPC::Client::VERSION . ' beta ',
        timeout => 10,
    );
...
sub call {
    ...
    if ($result->is_success) {
        ...
        if ($service) {
            return Datacoin::JSON::RPC::ServiceObject->new($result, $self->json);
        }

        return Datacoin::JSON::RPC::ReturnObject->new($result, $self->json);
    }
    elsif ($result->content =~ /^Can't connect to/) {
        my $s = $result->content;
        $s =~ s/\n/\\n/g;
        $result->content("{\"error\":\"$s\"}");
        return Datacoin::JSON::RPC::ReturnObject->new($result, $self->json);
    } else {
        return Datacoin::JSON::RPC::ReturnObject->new($result, $self->json);
    }
...
sub _post {
    ...
    if ($obj->{version} eq '1.0') {
        ...
        if (exists $obj->{id}) {
            ...
            $obj->{id} = $self->id || ($self->id('Datacoin::JSON::RPC::Client'));
        }
    }
    else {
...
package Datacoin::JSON::RPC::ReturnObject;

$Datacoin::JSON::RPC::ReturnObject::VERSION = $JSON::RPC::VERSION;
...
package Datacoin::JSON::RPC::ServiceObject;

use base qw(Datacoin::JSON::RPC::ReturnObject);
...
Datacoin::JSON::RPC::Client - Perl implementation of JSON-RPC client

=head1 SYNOPSIS

   use Datacoin::JSON::RPC::Client;
   
   my $client = new Datacoin::JSON::RPC::Client;
...
=head1 Datacoin::JSON::RPC::Client

=head2 METHODS

=over

=item $client = Datacoin::JSON::RPC::Client->new

Creates new Datacoin::JSON::RPC::Client object.
...
the module sets 'Datacoin::JSON::RPC::Client' to it.
